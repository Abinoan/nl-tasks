...
@section('prefixo-rota', 'atividades')
...
@section('cadastro-inputs')
    ...
    <div class="form-group">
        ...
        <input 
            ...
            placeholder="Título da Atividade" value="{{$registro->titulo}}"
        ...
        <div class="form-group col col-sm-12 col-lg-6">                
            ...
            <select name="projeto_id" id="projeto_id" class="form-control">
                @foreach ($projetos as $projeto)
                    <option 
                        value={{$projeto->id}}
                        @if ($projeto->id == $registro->projeto_id)
                            selected                        
                        @endif    
        ...
        <div class="form-group col-sm-12 col-lg-6">                
            ...
            <select name="situacao_id" id="situacao_id" class="form-control">
                @foreach ($situacoes as $situacao)
                    <option 
                        value={{$situacao->id}}
                        @if ($situacao->id == $registro->situacao_id)
                        selected
                    @endif                                        
                    >{{$situacao->descricao}}
                    </option>
    ...
    <div class="row">
        ...
        <div class="form-group col col-sm-6 col-lg-3">                
            ...
            <input 
                ...
                @if (date('U', strtotime($registro->inicio_estimado)) )
                    value="{{ date('Y-m-d', strtotime($registro->inicio_estimado)) }}"
                @endif
            >
            <div class="form-group">      
                <input 
                    type="time" class="form-control" id="idinicio_estimado_hora" name="inicio_estimado_hora" 
                    required style="width: 100px" value="{{$registro->inicio_estimado_hora}}"
                >
            </div>
        </div> 
        ...
        <div class="form-group col col-sm-6 col-lg-3">                
            ...
            <input 
                ...
                @if (date('U', strtotime($registro->inicio_real)) )
                    value="{{ date('Y-m-d', strtotime($registro->inicio_real)) }}"
                @endif
            >
            ...
            <div class="form-group">                
                <input 
                    ...
                    style="width: 100px" value="{{$registro->inicio_real_hora}}"
                >
            </div>
        </div>
        ...
        <div class="form-group col col-sm-6 col-lg-3">                
            ...
            <input 
                ...
                @if (date('U', strtotime($registro->termino_estimado)) )
                    value="{{ date('Y-m-d', strtotime($registro->termino_estimado)) }}"
                @endif    
            >
            ...
            <div class="form-group">                
                <input 
                    ...
                    style="width: 100px" value="{{$registro->termino_estimado_hora}}"
                >
            </div>                    
        </div>
        ...
        <div class="form-group col col-sm-6 col-lg-3">                
            ...
            <input 
                ...
                @if (date('U', strtotime($registro->termino_real)) )
                    value="{{ date('Y-m-d', strtotime($registro->termino_real)) }}"
                @endif    
            >
    ...
    <div class='card'>
        ...
        <div class="tab-content" id="tab-situacoes">
            <div class="tab-pane fade show active m-2" id="content-result-obtido" role="tabpanel" aria-labelledby="pills-home-tab">
                ...
                >{{$registro->resultado_encontrado}}</textarea>
            </div>
            <div class="tab-pane fade m-2" id="content-result-esperado" role="tabpanel" aria-labelledby="pills-profile-tab">
                ...
                >{{$registro->resultado_esperado}}</textarea>
            </div>
        </div>            
    </div>
